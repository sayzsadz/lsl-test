create or replace PROCEDURE  XXPBSA_T1_PROCESS_ITEMS(x_Transaction_Type varchar2, x_Segment1 varchar2, x_Description varchar2) AS
...
--          WHERE segment1 = 'SU_CATALOG';  -- Item Catalog Category Name
        ...
        -- Load l_item_tbl with the data
         l_item_tbl(l_rowcnt).Transaction_Type            := x_Transaction_Type; 
         l_item_tbl(l_rowcnt).Segment1                    := x_Segment1;            -- Item Number
         l_item_tbl(l_rowcnt).Description                 := x_Description;            -- Item Description
         l_item_tbl(l_rowcnt).Organization_Code           := 'SIO';                     -- Organization Code
 --        l_item_tbl(l_rowcnt).Template_Name               := 'Finished Good';          -- Item template  (** should be associated to ICC, Not mandatory) 
         ...
         l_item_tbl(l_rowcnt).template_name       := '@Product Family';  -- Item Catalog Group ID
