CREATE OR REPLACE PROCEDURE XXPBSA_JE_INTERFACE_BKDEPO(errbuf OUT NOCOPY VARCHAR2, retcode OUT NOCOPY VARCHAR2) 
IS        
        cursor cur
        is
...
FROM
  ...
  FROM BANKDEPOSITSUMMARY@DATABASE_LINK_EBS_APEX.LANKASATHOSA.ORG bd ,
       ...
       gl_code_combinations gcc
  WHERE bd.RECEIPTNUMBER = bdl.RECEIPTNUMBER
        AND UPPER(DECODE(gcc.segment6,NULL,'',apps.gl_flexfields_pkg.get_description_sql( gcc.chart_of_accounts_id,6,gcc.segment6))) like UPPER('%Floating%Cash%')
  )
  ) glq
WHERE 1 = 1;
