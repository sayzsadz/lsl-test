CREATE OR REPLACE PROCEDURE XXPBSA_JE_INTERFACE_TILL_RECON(errbuf OUT NOCOPY VARCHAR2, retcode OUT NOCOPY VARCHAR2) 
IS        
        cursor cur
        is
SELECT LEDGER_ID,
...
FROM
  ...
  ( SELECT DISTINCT bd.RECEIPTNUMBER,
    ...
    BD.BANKDEPOSITSDATE REFERENCE1 ,                 --  i.REFERENCE1
  ...
  WHERE bd.RECEIPTNUMBER = bdl.RECEIPTNUMBER
        AND UPPER(DECODE(gcc.segment6,NULL,'',apps.gl_flexfields_pkg.get_description_sql( gcc.chart_of_accounts_id,6,gcc.segment6))) like UPPER('BOC')
  ) 
  UNION ALL
  ( SELECT DISTINCT '0' RECEIPTNUMBER,
    2021 LEDGER_ID,                     --  SELECT * FROM GL_SETS_OF_BOOKS
    'Y' STATUS,                         --  i.STATUS
    2021 SET_OF_BOOKS_ID,               --  SELECT * FROM GL_SETS_OF_BOOKS      (Trading Companies SOB)
    'Manual' USER_JE_SOURCE_NAME,       --  SELECT * FROM GL_JE_SOURCES WHERE JE_SOURCE_NAME LIKE 'Manual'
    ...
    BDL.CASHAMOUNT ENTERED_CR ,   --  i.ENTERED_CR
    0 ACCOUNTED_DR , --  i.ACCOUNTED_DR
    BDL.CASHAMOUNT ACCOUNTED_CR , --  i.ACCOUNTED_CR
    XXPBSA_GL_CURR_OPERIOD PERIOD_NAME , --  i.PERIOD_NAME     (PERIOD SHOULD BE OPEN )
  ...
  FROM BANKDEPOSITSUMMARY@DATABASE_LINK_EBS_APEX.LANKASATHOSA.ORG bd ,
       ...
       gl_code_combinations gcc
  WHERE bd.RECEIPTNUMBER = bdl.RECEIPTNUMBER
        AND UPPER(DECODE(gcc.segment6,NULL,'',apps.gl_flexfields_pkg.get_description_sql( gcc.chart_of_accounts_id,6,gcc.segment6))) like UPPER('%Other%Income%')
  )
  ) glq
WHERE 1 = 1;
